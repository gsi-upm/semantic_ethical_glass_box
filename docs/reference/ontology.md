...
If you are looking for a practical mental model, start with [Core Concepts and Roles](../getting-started/core-concepts-and-roles.md).
If you want the namespace and entity details, stay here.
...
When multiple observations may refer to the same real-world event, SEGB can connect them to one canonical shared
context.

### Temporal Evidence

Timestamps are important because many analyses depend on ordering:

- conversation order,
- emotion timelines,
- robot state changes,
- shared-context matching windows.
...
When debugging real data, do not try to understand the whole ontology at once. Start by checking:

1. which resources were created,
2. whether they are typed correctly,
3. whether the important links exist between them.

That is usually enough to understand why a UI page or query behaves the way it does.
